/** Ported from `admin.edition_unpublish` (`app/routes/admin.py:340-350`). */
...
export async function POST(
  ...
  await prisma.edition.update({
    where: { id },
    data: { status: EDITION_STATUS_DRAFT, publishedAt: null },
  });

  return flashRedirect(request, "/admin/editions", [
    { type: "success", text: `Edition '${edition.title}' moved back to draft.` },
  ]);
}
