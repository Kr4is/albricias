...
/**
 * Article detail. Ported from `public.article_detail`
 * (`app/routes/public.py:212-239`) and `app/templates/article.html`.
 *
 * Two notes on the original template:
 *  - `article.image_caption` does not exist on the Article model, so the
 *    `{% if %}` guarding the `<figcaption>` was always falsy and no caption
 *    ever rendered. Dropped rather than invented.
 *  - the inline `<style>` block for the markdown drop cap now lives in
 *    globals.css as `.first-p-drop-cap`.
 */
...
import { toEditionHeaderInfo } from "@/lib/issue-view";
...
async function loadArticle(articleId: string) {
  ...
  if (article === null || !isPublished(article.edition)) return null;
  return article;
}

export async function generateMetadata({
  params,
}: PageProps<"/article/[articleId]">): Promise<Metadata> {
  const { articleId } = await params;
  const article = await loadArticle(articleId);
  return {
    title: article ? `${article.title} - Albricias` : "¡Albricias!",
  };
}
...
export default async function ArticlePage({
  ...
  return (
    <NewspaperShell
      endpoint="public.article_detail"
      ...
      <div className="grid grid-cols-12 gap-6 lg:gap-8 relative">
        ...
        <aside className="col-span-12 lg:col-span-3 lg:border-r lg:border-stone-300 lg:pr-6 flex flex-col order-2 lg:order-1">
          ...
          <div className="mb-8 border-b border-ink pb-4">
            <a href={`/edition/${edition.id}`} className="flex items-center group">
          ...
          <div className="mb-8">
            ...
            <ul className="space-y-3 text-xs font-headline">
              {siblings.map((item) => (
                ...
                >
                  <a href={`/article/${item.id}`} className="block leading-snug">
                    {item.title}
                  </a>
                </li>
              ))}
